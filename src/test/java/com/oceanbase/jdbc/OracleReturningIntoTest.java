...
public class OracleReturningIntoTest extends BaseOracleTest {
    ...
    @Test
    public void testReturningStreamData() {
        try {
            Connection conn = setConnectionOrigin("?useServerPrepStmts=true&usePieceData=true");
            createTable("t_return_clob", "c1 clob, c2 blob");
            JDBC4ServerPreparedStatement ps = (JDBC4ServerPreparedStatement) conn
                    .prepareStatement("insert into t_return_clob values(?, ?) returning c1,c2 into ?,?");

            ps.setCharacterStream(1, new StringReader("abc"));
            ps.setBinaryStream(2, new ByteArrayInputStream("abc".getBytes()));
            ps.registerReturnParameter(3, Types.CLOB);
            ps.registerReturnParameter(4, Types.BLOB);
            ps.execute();
    ...
    @Test
    public void testReturningBatchBySQLForDima2024110800105027138() throws SQLException {
        ...
        try {
            JDBC4ServerPreparedStatement ps = (JDBC4ServerPreparedStatement) conn
                .prepareStatement("insert into t_rb values(?) returning c1 into ?");
            ps.setInt(1, 10);
            ...
            assertArrayEquals(new int[] { -2, -2 }, ints);
            ...
            ps = (JDBC4ServerPreparedStatement) conn
                .prepareStatement("update t_rb set c1 = ? returning c1 into ?");
            ps.setInt(1, 11);
            ps.registerReturnParameter(2, Types.INTEGER);
            ps.addBatch();
            ps.setInt(1, 22);
            ...
            assertEquals(4, ps.getUpdateCount());
            conn.commit();
            assertArrayEquals(new int[] { -2, -2 }, ints);
            ...
            ps = (JDBC4ServerPreparedStatement) conn
                .prepareStatement("delete from t_rb where c1 = ? returning c1 into ?");
            ps.setInt(1, 11);
            ps.registerReturnParameter(2, Types.INTEGER);
            ps.addBatch();
            ps.setInt(1, 22);
